...
## [0.1.23] - 2026-08-12
### Changed
- **`lfs-06` — "The deeper reason that table has a second column".** New section explaining *why* the checks-vs-blind-spots table exists at all: every check in the lab measures a **continuous** quantity (nats, PCC, relative error) while the shipped output is `argmax`, a **step function**, so at a near-tie the map from logits to tokens is a discontinuity that smooth agreement cannot cross. Grounded in a real divergence from the companion project where the top two logits were `' She'` 12.3750 (p=0.574) and `' Lily'` 11.9375 (p=0.370) — a 0.4375-logit margin, ~9 bfloat16 ulps — so **PCC 0.9940–0.9998 and "the generated text is wrong" were true simultaneously**. Covers the three properties that sharpen this on Tenstorrent<sup>®</sup> hardware: block float (`bfp8`/`bfp4`) shares one exponent across a 32-value tile so error is *correlated between neighbours* rather than independent (measured: raising precision bought ~1 token of agreement, so treat it as a hypothesis and A/B it); decode is a feedback loop that amplifies per-step error, which is exactly why teacher-forced decode checks pass while real generation degrades; and small models sit on a knife edge — a ~22M model at a fraction of an epoch has **21% of positions within 0.5 logits** where an 8B model flips nothing on the same absolute error. Includes the counter-intuitive result that training the same architecture properly made the distribution **flatter** (near-ties 21% → 32%), because lower loss means better probability *assignment*, not wider top-1/top-2 gaps. Closes with the rule: if the deliverable is tokens, measure tokens.

## [0.1.22] - 2026-08-12
### Changed
- **`lfs-06` — CORRECTION: the RMSNorm blind spot does not close when you fix the frozen gammas.** The lab's punchline held that swapping two RMSNorm layers cost `+0.0000` *because* all 13 gammas were frozen at exactly 1.0 — true, but it left readers with the reasonable inference that repairing that training bug would restore the check. The companion project ran the experiment: after retraining with `stochastic_rounding: true` (gamma sd `0.047`–`0.158` instead of `0.0`), the canonical block0↔block1 swap measures **+0.0065** and the block3 `attention_norm`↔`mlp_norm` swap **+0.0018** — non-zero at last, but **~31× below the 0.2-nat gate** and 45× below the model's own per-window noise (sd `0.29`); on the smaller swap 10 of 32 windows actually improved. New subsection **"The follow-up experiment — and why the obvious fix isn't one"** states the uncomfortable conclusion plainly: a norm mis-mapping slips past every loss-based gate *both* before the fix (tensors identical) and after (effect real but tiny) — two independent causes, and repairing the dramatic one did nothing for the second. Framed as the lab's most transferable lesson: a check reading `0.0000` is obviously broken and gets investigated, while one reading `+0.0065` looks like it works — it moved, right sign, even significant at t ≈ 5.6 across 32 windows — and is still far too small to gate on.
- **`ct7` — `num_groups` quietly decides which hardware you can serve on.** New callout where GQA is introduced, covering the one place in that conceptual tour where an architecture choice reaches into deployment. `tt_transformers` asserts `n_heads % cluster_shape[1] == 0` **and** `n_kv_heads % cluster_shape[1] == 0` (`model_config.py:687-691`, re-asserted `attention.py:234-237`); `num_heads` is usually a friendly power of two but `num_groups` often isn't, and being the smaller number it constrains harder. Worked example: `num_groups: 3` (a reasonable 2:1 ratio against 6 query heads) is servable **only on a single chip** — N300/P300, T3000, and Galaxy submeshes all fail — because 3 shares no factor with any common mesh width. Notes that `cluster_shape` is read from the live `mesh_device.shape` (`model_config.py:532`) rather than a fixed per-board constant, and advises picking `num_groups` divisible by the target mesh width while it is still a one-line config decision.
- **`lfs-05` — added "What fixing it was worth" to the frozen-gamma section.** The lab documented the bug and both remedies but never said what repairing it bought, which matters because "13 layers weren't training" sounds catastrophic while the loss curve looked healthy throughout. Retraining the identical architecture on the identical corpus changing only `stochastic_rounding`: held-out loss at the same step 3000 improves **1.878 → 1.783**, best loss reaches **1.456**, and a paired evaluation over 32 identical held-out windows shows **+0.45 nats with all 32 windows improving** (per-window losses correlating at r = 0.97, so the pairing is real). Headline: the fixed run passed the frozen run's *final* loss **before step 3000** — a ~7× compute saving from one flag. Includes two deliberate caveats so the section doesn't oversell: the 1.878 → 1.456 figure conflates the fix with 7× more training (the clean comparison is the step-3000 row), and generated text is only *subtly* better to read. Notes the post-step-10,000 plateau into a 1.46–1.59 band as the signature of a **data**-bound model, matching `ct8`'s ~80M finding.

## [0.1.21] - 2026-08-12
### Changed
- **`lfs-05` — corrected the "physical dimension" passage, which described a single-chip run in the plural.** It said "the Blackhole chips settle in around 70–81 °C ... as the mesh works hard", but every run in that lab is `mesh_shape [1, 1]` — one chip works and the rest idle. Now describes the working chip singular, and adds a short **"One chip, and you can see it"** note showing how to observe that in telemetry, with measurements from a real training run on a TT-QuietBox<sup>®</sup> 2: **82 W / 73 °C on the working chip against 61–73 W / 63–68 °C on the idle three**. Includes the non-obvious part — idle Blackhole<sup>®</sup> holds its clock at 1350 MHz, so idle chips stay warm and the *power* gap is the clearer tell, not temperature. Links out to `ct5-multi-device-training` for spreading the load.

## [0.1.20] - 2026-08-12
### Added
- **New lesson `lfs-06-verify-your-model` — "Prove It's Right: Verifying a Model You Trained"** (order 206, closing the Build an LLM from Scratch arc after `lfs-05`). The arc previously ended at "you trained it"; this lab answers "how do you know it's right?" Its spine is that **"it loads" and "it generates fluent text" are not evidence of correctness** — evidenced by a converted model that passed four independent checks (loads as `LlamaForCausalLM` at 22.025088M params, embedding/`lm_head` tied, next-token entropy 4.75 nats against a uniform 10.37, fluent generated text) and was still computing the wrong function because of a RoPE interleaved-vs-split-halves row-layout mismatch, caught only by held-out loss (3.20 vs the training run's 1.8781). Covers five techniques with measured numbers: `ln(vocab_size)` at initialization, held-out loss against the training run (including the HF double-shift trap and the noise floor: per-window sd ≈ 0.29, 8-window SE ≈ 0.11), resume continuity, deliberate ablation (with the calibration table showing a 0.2-nat gate catches a flipped RoPE at 3.13 but two swapped RMSNorm layers at +0.0000), and independent reimplementation (≈1e-5 logit agreement from a pure-NumPy `ttml` forward written from the C++ sources). Also covers three silent conversion gotchas: weight tying leaves no `tok_emb` tensor, `kv_linear` is a fused K+V projection needing a K-first split, and checkpoint tensors stream in declaration order rather than sorted order. Numbers come from the companion project [tt-nanollama3](https://github.com/tsingletaryTT/tt-nanollama3), which carried this arc through to a trained, converted, numerically verified model on Blackhole<sup>®</sup>.

### Changed
- **`lfs-05` — added a frozen-RMSNorm-gamma gotcha section.** A real training failure a reader of that lab will hit: after 3000 steps all 13 RMSNorm gammas were exactly 1.0 (`mean=1.0, sd=0.0`) while the loss dropped normally (10.6875 → 1.9219). The gradients were real (AdamW `exp_avg` absmax ≈ 3.6e-4 for block 0, ≈ 2.5e-3 for the final norm) — the parameters are bf16 at 1.0, where one ulp is 0.0039, so every ~3e-4 update rounds straight back to 1.0 and is discarded. `stochastic_rounding` defaults to `false` in `AdamWConfig` and still does as of v0.75.0, so this is not fixed by upgrading; the section gives both shipped remedies with config snippets (`stochastic_rounding: true`, already used by two shipped `tt-train` configs, or `type: AdamWFullPrecision` for fp32 master weights) and the check that actually detects it (inspect parameter statistics, not the loss — `sd == 0.0` is the tell).
- **`lfs-05` — `nextLesson` now points at `lfs-06-verify-your-model`**, with a closing hand-off to the new verification lab.
