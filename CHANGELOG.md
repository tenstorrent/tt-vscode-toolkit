...
### Added
- **Redirect stubs for retired lesson URLs.** The two published Koyeb lesson URLs were indexed and cited in `llms.txt`, so `scripts/build-web.js` now emits a stub page per retired slug (`meta refresh` + canonical + `noindex`) pointing at the lesson catalog with a one-line note explaining the removal. GitHub Pages has no server-side redirects and the site sets `.nojekyll`, so a static stub is the only mechanism available. Add to the `REDIRECTS` map whenever a published slug is removed.
...
### Added
- **New lesson `lfs-06-verify-your-model` — "Prove It's Right"** (order 206), closing the Build-an-LLM-from-Scratch arc. Its spine: *"it loads" and "it generates fluent text" are not correctness* — a converted model passed four independent checks and was still computing the wrong function (a RoPE row-layout mismatch, caught only by held-out loss). Walks five verification techniques with measured numbers — init entropy, held-out loss vs. the training run, resume continuity, deliberate ablation, and an independent pure-NumPy reimplementation — plus three silent checkpoint-conversion gotchas. Numbers from the companion project [tt-nanollama3](https://github.com/tsingletaryTT/tt-nanollama3).

### Changed
- **`lfs-05` hardened with real QB2 training findings** — the frozen-RMSNorm-gamma gotcha (bf16 updates round straight back to 1.0 unless `stochastic_rounding: true`), what fixing it is worth (held-out loss 1.878 → 1.783 at the same step — by step 3000 the fixed run already beat the frozen run's *best* loss), and a corrected single-chip telemetry passage (one chip works at ~82 W / 73 °C while the other three idle). `nextLesson` now hands off to `lfs-06`.
- **Verification nuances in `lfs-05`/`lfs-06`** — a correction that the RMSNorm blind spot does *not* fully close even after fixing the frozen gammas (the swap still measures ~31× below the gate), and why continuous checks (PCC, nats) can read healthy while the `argmax` output is wrong: if the deliverable is tokens, measure tokens.
- **`ct7` — GQA `num_groups` quietly constrains deployable hardware.** `tt_transformers` requires both `n_heads` and `n_kv_heads` divisible by the mesh width, and `num_groups` (the smaller, often non-power-of-two number) binds hardest — e.g. `num_groups: 3` is servable only on a single chip. Pick it divisible by your target mesh width while it's still a one-line config decision.

### Fixed
- **QB2 agent-server buttons no longer swallow the server start on a failed reset.** The board reset was chained with `&&`, so any `tt-smi -r` failure — the tool not being on `PATH` (the case this lesson's own troubleshooting section documents), or the device still held by a previously running container — silently prevented the server from starting at all. The reset is now non-fatal and prints a warning instead.
- **`Clone tt-agents` now re-points `origin` when updating an existing clone.** The repo moved orgs; the update path only ran `git pull`, so anyone who used this button previously stayed pinned to the old org indefinitely and never received `tt_agents_common.py`. The command now branches on whether a repo already exists (instead of discarding clone stderr, which hid real DNS/auth/permission failures behind a confusing pull error) and rewrites the remote before pulling.
- **bf16 ulp figures corrected in `lfs-05` and `lfs-06`.** bf16 stores **7** mantissa bits, so ulp(1.0) = 2⁻⁷ = 0.0078125; the previously quoted 0.0039 is the *half*-ulp round-to-nearest threshold, and the text now distinguishes the two. The stochastic-rounding probability follows: 3e-4 against that ulp lands roughly 1 step in 26, not 1 in 13. In `lfs-06`, a 0.4375-logit margin at ~12.4 sits in the [8, 16) binade where one ulp is 0.0625, making it exactly **7** ulps rather than 9.
- **Removed an unsupported compute-saving multiple in `lfs-05`.** "Passed the frozen run's final loss before step 3000" does not imply a 7× saving against a 3000-step baseline (that would need a crossing near step 430); the 7× belonged to a different quantity — the fixed run trained ~7× longer. The claim is now stated as what the measured table actually supports.
- **Ablation table in `lfs-06` no longer mixes absolute losses with deltas** in one column. Rows are now sorted by severity with separate `Held-out loss` and `Δ vs baseline` columns, so reading the table against the stated 0.2-nat gate tolerance is valid for every row.
- **`qb2-local-agents` thinking/env-var callout corrected to match the repo.** `tt_agents_common.py` is imported by the five framework demos (`01`–`05`), not literally every script: `00_verify_tools.py` implements the same non-thinking default inline and `06_landscape_svg.py` hardcodes it. `TT_ENABLE_THINKING` / `TT_MAX_TOKENS` are honoured by `00`–`05` but ignored by `06`, and the documented `TT_MAX_TOKENS` default is 3072. Adds a pointer for readers whose clone predates the shared helper.
