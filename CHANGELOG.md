...
- **A gate at the top of Step 0** so nobody runs the wheel install on a machine that already has a working `ttnn`. On a TT-QuietBox 2 `ttnn` is an editable install linked to a `~/tt-metal` source tree, and installing a wheel over it yields a wheel from one release driving a `models/` tree from another. The previous wording said "the standard and QB2 images", which a reader on QB2 *hardware* could reasonably misread as being about their box; it now distinguishes image names from hardware explicitly.
- Added a `models/` reachability check to the verification step. On a wheel-based setup that is a genuinely separate failure mode from `ttnn` not importing, and previously nothing caught it until vLLM failed to inspect the model class.

### Fixed
- **The lesson assumed `tt-vllm` exists everywhere.** It is an alias supplied by some tt-developer-image variants, not part of vLLM or the plugin, and the QB2 variant deliberately omits it so it mirrors a real TT-QuietBox 2. The activation guidance is now a labelled list covering the standard and latest-metal variants (`tt-vllm`), the QB2 variant (`~/tt-metal/build/python_env_vllm`), a QB2 pre-installed image, a tt-metal source build, and cloud installs — with a note on what to substitute when `tt-vllm` is not a command.
- Corrected the prerequisites line, which claimed TT-Metalium had to be "installed and working (latest main branch)". That overstated it: what is needed is an importable `ttnn` and a `models/` tree, obtainable without compiling anything.
- Replaced ten duplicated inline activation comments with a pointer to the single canonical activation section, so the list cannot drift out of sync in ten places.

- **Re-authored the migration section around observed state rather than assumed paths.** It previously keyed off `~/tt-vllm`, which is only one way people ended up on the fork — others cloned elsewhere, inherited a preinstalled vLLM, or have nothing. It now opens with a diagnostic that reports what the environment actually has, followed by a table mapping each observed state to an action, and a removal procedure that works wherever the old install came from (including when the clone an editable install pointed at is already deleted). The diagnostic deliberately never imports vLLM, because a stale install commonly fails on import and that would hide the information needed to fix it.
- **The diagnostic detects duplicate and malformed `dist-info`**, which blocks a clean uninstall. Found on a real TT-QuietBox 2: two `vllm-*.dist-info` directories, one with an invalid `name` entry that pip skips, which made the version lookup report "not installed" while the module was plainly importable. Version detection now scans all distributions instead of asking for one, so a single corrupt entry cannot hide the rest, and reports only *distinct* versions (the same version legitimately appears twice when a path is on `sys.path` twice).

### Notes
- Which images the lesson works on, as built: the tt-developer-image **latest-metal** variant ships an importable `ttnn` and can skip Step 0 entirely. The **standard** and **QB2 image variants** default to `TT_METAL_BUILD=checkout`, which has no compiled `ttnn` — on those, Step 0's wheel path (seconds) or a `TT_METAL_BUILD=full` rebuild is required.
- **tt-installer does not and cannot perform the plugin install.** Its scope is `kmd`, `sfpi`, `tt-flash`, `tt-smi`, `tt-topology`, `hugepages` and four containers; it contains no vLLM references and never manages `ttnn` or a tt-metal Python environment. The two are orthogonal, so the lesson's steps cannot conflict with a tt-installer install — and there is no tt-installer flag to look for. Its only route to vLLM serving is `--install-inference-server`, which is the separate tt-inference-server Docker path.

## [0.1.21] - 2026-08-03
### Changed
- **Repointed the vLLM install path at the standalone plugin repo.** Tenstorrent's vLLM platform plugin's official home is now `github.com/tenstorrent/vllm-tt-plugin`, which works against **upstream** vLLM — its own installer pins `vllm==0.24.0` — so no Tenstorrent fork is involved. v0.1.20 had documented the in-fork copy (`tenstorrent/vllm` `dev` branch, `plugins/vllm-tt-plugin`); that copy still exists and tt-metal's README still links to it, but it is being retired. `tenstorrent.cloneVllm` now clones the standalone repo and `tenstorrent.installVllm` runs its `docs/install-vllm-tt.sh`.
- Install guidance now covers `docs/vllm-overrides.txt`, which is load-bearing: it pins `numpy>=1.24.4,<2` plus `opencv-python-headless==4.11.0.86`. ttnn requires numpy<2 while vLLM's opencv floor wants numpy>=2, so without the override the install appears to succeed and then `import ttnn` fails.
- Documented three dependencies the upstream installer does not cover when the target environment is not already a full tt-metal env, each a real observed failure: `pandas seaborn ml_dtypes graphviz networkx` (ttnn's tracy tooling; surfaces as an opaque "error while initializing the extension"), `torchvision` (transformers' pixtral processor; surfaces as "Model architectures [...] failed to be inspected"), and `pytest` (tt-metal's `models/common/utility_functions.py` imports it at module scope).
- **Documented that `HF_MODEL` is required whenever `--model` is a local path.** tt-metal's `tt_transformers` uses `HF_MODEL` as its checkpoint directory, so it must be either a HuggingFace `org/name` or the path to downloaded weights; omitting it fails with "Please set HF_MODEL to a HuggingFace name".

### Fixed
- **Softened the Qwen3-0.6B recommendation for the native vLLM path.** `0.6B` appears nowhere in tt-metal's `models/tt_transformers/`; the only Qwen3 supported there is Qwen3-32B. The plugin maps Qwen3 by architecture so the model loads and runs, but it has no validated model parameters. Llama-3.1-8B (n150/p100/p150) and Qwen3-32B (QuietBox-class) are the documented choices.
- Corrected the accuracy note about upstream listing: Tenstorrent is absent from the "Universal Compatibility" section on vllm.ai, which names NVIDIA, AMD, Huawei, AWS, Google, IBM, Intel, Apple, Baidu, and Cambricon.

### Notes
- **Validated on a TT-QuietBox 2 (4x P300C)** with upstream `vllm==0.24.0` and the standalone plugin: the plugin installs and activates (`Platform plugin tt is activated`), `TTPlatform` is selected, both entry points register, `MESH_DEVICE=P300x2` resolves to a `(1,4)` mesh, the driver opens all four chips, weights load, KV cache allocates on-mesh, and the OpenAI-compatible server serves `/v1/models` and `/v1/completions`.
- **Generation quality remains unvalidated: output is degenerate**, collapsing into repetition. Reproduced across five configurations — the fork's `dev` HEAD and upstream `0.24.0`; Qwen3-0.6B, Llama-3.1-8B-Instruct, and Qwen3-32B; single-chip `P100` and four-chip `P300x2` — with byte-identical output across both vLLM versions. That rules out the plugin, the install path, the mesh, and the model. The remaining invariant is the host tt-metal build, a dev build far from any release in tt-metal's LLMs table (which pins tt-metal release to vLLM commit). `p300x2` is therefore still absent from `validatedOn`; retest against a paired tt-metal release before trusting output quality.

## [0.1.20] - 2026-08-03
### Fixed
- **`deploy-to-koyeb` cloned the vLLM fork's default branch, producing an image with no Tenstorrent support.** The fork's default branch now tracks upstream vLLM, so the Dockerfile needed `--branch dev` plus the separate plugin install step. Same missing `--branch dev` corrected in the CONTRIBUTING vendor-clone instructions.
- **FAQ recommended `--tensor-parallel-size` for n300 and T3000, which the TT platform rejects outright.** Replaced with the correct mechanism — multi-chip execution is selected by the `MESH_DEVICE` mesh shape — and documented the full valid value set with mesh shapes. The extension's own hardware config map had the same defect and no longer emits tensor-parallel flags.
- **`VLLM_TARGET_DEVICE` was documented as a runtime variable with value `tt`.** It is build-time only, and the correct build value is now `empty`, because the `tt` platform is contributed by the plugin at runtime rather than compiled in. Corrected in the version-compatibility and step-zero reference pages.
- **`vllm-production` claimed you must not invoke the OpenAI API server directly "because TT models aren't registered".** That has not been true since model registration moved to plugin entry points. Replaced with the real failure mode (plugin missing or not discovered) and concrete discovery checks.
- **TT-QuietBox 2 was described as "4 independent single-chip devices, use device 0 only".** It is two P300 cards — four Blackhole chips in a 2x2 mesh — and the plugin exposes it as `MESH_DEVICE=P300x2`.
- Corrected a performance attribution in the version-compatibility page: the batch-32 22.1 T/S/U / 707.2 T/S figures are n300 numbers for Qwen 2.5 7B, not TT-QuietBox numbers.
- `GALAXY` is not a valid `MESH_DEVICE` value anywhere; both tt-metal and the vLLM plugin use `TG`.
- Removed roughly 139 lines of wholesale duplicated content from the FAQ — the entire "Environment Reference" section appeared twice.

### Changed
- **Re-authored `vllm-production` around the vLLM platform plugin** rather than a whole-fork build. Installation is now the upstream two-part flow (`VLLM_TARGET_DEVICE=empty` base install, then `plugins/vllm-tt-plugin`), driven by the shipped `install-vllm-tt.sh`. Documents plugin entry points and discovery verification, the `--additional-config '{"tt": {...}}'` namespace that replaced `--override_tt_config` / `--plugin-config`, unsupported features, `vllm bench serve` benchmarking, and a migration note for pre-plugin `~/tt-vllm` checkouts.
- **Retired the custom `start-vllm-server.py` starter script** and the `tenstorrent.createVllmStarter` command. The script existed solely to call `ModelRegistry.register_model()`, which the plugin now handles via entry points, so lessons and extension commands use plain `vllm serve`. `setup-vllm-env.sh` and `setup-aider.sh` updated accordingly.
- **Extension vLLM hardware map gained `P150`, `P300`, and `P300X2` (TT-QuietBox 2)**, records each shape's chip mesh, and carries the `MESH_DEVICE` spelling explicitly (the env value is `P300x2`, not `P300X2`). `installVllm` now performs the two-part plugin install and verifies the plugin imports.
- **Updated `tt-inference-server` lesson for v0.19.0** — `--tt-device` replaces `--device`, `p300x2` added as a first-class TT-QuietBox 2 target, the deleted `setup.sh` and removed `reports`/`tests` workflows dropped, model status vocabulary surfaced, both authentication schemes documented, and the `sudo chown 1000` host-volume requirement noted.
- FAQ's vLLM model-support answer now reflects the families the plugin actually registers, including Gemma 3, DeepSeek V3, GPT-OSS, and the vision-language models, plus `EXTRA_MODELS_DIR` for unregistered models.
- CLAUDE.md gained a vLLM plugin section, tt-inference-server v0.19.0 gotchas, and a correction: tt-metal has shipped the `p300_x2` (2x2) and `p150_x4` mesh-graph descriptors since October 2025 — the "only 8-dev and 32-dev ship" caveat applies to ttml/tt-train's bundled subset, not tt-metal.

### Notes
- Tenstorrent is **not** currently a listed vLLM plugin upstream and no TT plugin is published to PyPI. Content describes it accurately as a conformant out-of-tree platform plugin installed from source. Four separate TT vLLM plugins exist and tenstorrent/vllm#452 tracking the ambiguity is still open, so the lessons stay on the in-fork plugin that tt-metal's README documents and tt-inference-server's images build.
- `MESH_DEVICE=P300x2` is documented as plugin-supported but is **not** claimed as validated; no 4-chip vLLM serving run has been performed. `validatedOn` metadata is unchanged.
