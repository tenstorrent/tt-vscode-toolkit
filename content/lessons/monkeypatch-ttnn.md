...
dependency-free helper. The button below drops the harness folder
(`tt_patches.py`, its hardware-free tests, and a README) into
`~/tt-scratchpad/monkeypatch/` so you can copy it into your own project:

[📋 Copy the tt_patches harness to ~/tt-scratchpad](command:tenstorrent.monkeypatch.copyHarness)
...
It's about 150 lines of plain Python with no third-party dependencies — nothing
hidden. Read the whole thing before you run it:

<details>
<summary>📄 Full source of <code>tt_patches.py</code> (click to expand)</summary>
