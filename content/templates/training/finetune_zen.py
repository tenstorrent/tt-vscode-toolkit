#!/usr/bin/env python3
"""
Zen Master Fine-tuning Script

Fine-tunes TinyLlama-1.1B on witty one-liners and koans using TT-Train.
Turns a general-purpose model into a Zen Master that responds in zingers.
...
Usage:
    python finetune_zen.py --config configs/finetune_zen_n150.yaml
"""
...
class ZenDataset(Dataset):
    """
    Dataset for Zen Master one-liners.
...
class CollateFn:
    """
    Collate function for batching Zen Master examples.
...
def generate_zen_response(
    ...
    """
    Generate a Zen Master response to a question.
...
def validate(
    ...
    with open(output_file, "a+") as val_file:
        ...
        for i, question in enumerate(test_questions):
            ...
            response = generate_zen_response(
                tt_model,
                tokenizer,
                question,
                max_sequence_length,
...
def train(args):
    """
    Main training loop for Zen Master fine-tuning.
    """

    print("="*60)
    print("🧘 Zen Master Fine-tuning")
    print("="*60)
    print()
    ...
    train_dataset = ZenDataset(args.train_data, tokenizer)

    val_dataset = None
    if args.val_data:
        print(f"Loading validation dataset: {args.val_data}")
        val_dataset = ZenDataset(args.val_data, tokenizer)
    else:
    ...
    with open(val_output, "w") as f:
        f.write("Zen Master Fine-tuning - Validation Log\n")
        f.write("="*60 + "\n\n")
    ...
    plt.xlabel("Steps")
    plt.ylabel("Loss")
    plt.title("Zen Master Fine-tuning")
    plt.legend()
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune TinyLlama on Zen Master dataset")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--train-data",
        type=str,
        default="zen_dataset_starter.jsonl",
