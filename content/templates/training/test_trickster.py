#!/usr/bin/env python3
"""
Trickster Inference Testing

Tests the fine-tuned Trickster model by generating responses
to sample prompts. Compares base model vs fine-tuned responses.

Usage:
    # Test fine-tuned model
    python test_trickster.py --model-path ./output/final_model --config configs/trickster_n150.yaml

    # Compare base vs fine-tuned
    python test_trickster.py --model-path ./output/final_model --config configs/trickster_n150.yaml --compare
"""
...
def test_model(args):
    """
    Test the Trickster model on sample questions.
    """

    print("=" * 70)
    print("🎭 Trickster Inference Test")
    print("=" * 70)
    print()
    ...
    # Test fine-tuned model
    print("\n" + "=" * 70)
    print("✨ TRICKSTER: Testing fine-tuned model")
    print("=" * 70)
    print("\nCreating model...")
    trickster_model = tt_model_factory.create_model()

    print(f"Loading fine-tuned weights from {args.model_path}")
    trickster_model.load_from_safetensors(args.model_path)
    ...
    if args.output:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write("Trickster Inference Test Results\n")
            f.write("=" * 70 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Test fine-tuned Trickster model")
    parser.add_argument(
        "--model-path",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="trickster_test_results.txt",
