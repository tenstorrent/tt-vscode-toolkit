#!/usr/bin/env node
/**
 * First mention of "Wormhole" per page → Wormhole<sup>™</sup>
 * Skips YAML front matter, inline `code`, markdown link URLs, and HTML script/style.
 *
 * Usage:
 *   node scripts/add-wormhole-trademark.js          # apply
 *   node scripts/add-wormhole-trademark.js --check  # exit 1 if any file needs update
 */
...
const MARK = 'Wormhole<sup>™</sup>';
const WORD_RE = /\bWormhole\b/;
...
const FILES = ['README.md', 'CLAUDE.md', 'CHANGELOG.md', 'COMMUNITY_GUIDELINES.md'];
...
function transformMarkdownLine(line) {
  ...
  for (let i = 0; i < parts.length; i += 2) {
    ...
    if (replaced) {
      parts[i] = segment;
      return parts.join('').replace(/\x00U(\d+)\x00/g, (_, idx) => urlSlots[Number(idx)]);
    }
  }
  return line;
}
...
function main() {
  ...
  if (checkOnly) {
    if (pending > 0) {
      console.error(`\n${pending} file(s) missing first-mention Wormhole<sup>™</sup>.`);
      process.exit(1);
    }
    console.log('OK: all pages with Wormhole have first-mention trademark.');
    process.exit(0);
  }

  console.log(pending ? `\n${pending} file(s) updated.` : 'No Wormhole pages needed updates.');
}
