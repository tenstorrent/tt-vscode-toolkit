...
async function main() {
  ...
  if (changeCount === 0 && isDryRun) {
    // In dry-run mode, exit early if no changes
    process.exit(0);
  }

  if (changeCount === 0 && !isDryRun) {
    // No registry changes, but still update README catalog
    console.log(`${colors.blue}📝 Registry is in sync, checking README.md...${colors.reset}\n`);
    const readmeUpdated = updateReadmeCatalog(oldRegistry);
    if (readmeUpdated) {
  ...
  try {
    ...
    // Update README.md with lesson catalog
    updateReadmeCatalog(newRegistry);
...
/**
 * Generate lesson catalog markdown for README
 */
function generateLessonCatalog(registry) {
  const categoryIcons = {
    'welcome': '👋',
    'first-inference': '🚀',
    'serving': '🏭',
  ...
  for (const category of registry.categories) {
    const lessons = lessonsByCategory[category.id] || [];
    const validatedCount = lessons.filter(l => l.status === 'validated').length;

    // Category header (category.title already includes emoji icon)
    markdown += `\n### ${category.title}\n`;
    markdown += `*${lessons.length} lesson${lessons.length !== 1 ? 's' : ''}, ${validatedCount} validated*\n\n`;

    // List lessons
    for (const lesson of lessons) {
      markdown += `- **${lesson.title}**`;

      // Add validated hardware badges
      if (lesson.validatedOn && lesson.validatedOn.length > 0) {
        const badges = lesson.validatedOn
          .map(hw => `\`${hw.toUpperCase()}\``)
          .join(' ');
        markdown += ` — ${badges}`;
      } else if (lesson.status === 'validated') {
        markdown += ` — *validated*`;
      } else if (lesson.status === 'blocked') {
        markdown += ` — *⚠️ blocked*`;
...
/**
 * Update README.md with generated lesson catalog
 */
function updateReadmeCatalog(registry) {
  const README_PATH = path.join(__dirname, '../README.md');

  console.log(`${colors.blue}📝 Updating README.md lesson catalog...${colors.reset}`);
  ...
  try {
    ...
    console.error(`${colors.red}❌ Failed to read README.md: ${error.message}${colors.reset}`);
    return false;
  }

  // Find markers
  const startMarker = '<!-- LESSON_CATALOG_START -->';
  const endMarker = '<!-- LESSON_CATALOG_END -->';

  const startIndex = readme.indexOf(startMarker);
  const endIndex = readme.indexOf(endMarker);

  if (startIndex === -1 || endIndex === -1) {
    console.error(`${colors.red}❌ Could not find catalog markers in README.md${colors.reset}`);
    console.log(`   Expected markers: ${startMarker} and ${endMarker}`);
    return false;
  }

  // Generate new catalog
  const catalogMarkdown = generateLessonCatalog(registry);
  ...
  try {
    ...
    console.log(`${colors.green}✅ README.md lesson catalog updated${colors.reset}\n`);
    return true;
  } catch (error) {
    console.error(`${colors.red}❌ Failed to write README.md: ${error.message}${colors.reset}`);
    return false;
  }
}
...
module.exports = { generateLessonEntry, parseFrontMatter, generateLessonCatalog, updateReadmeCatalog };
